...
(
  ...
  int length,  // numero de subunidades foneticas da transcricao da locucao
  struct modelo_HMM HMM,      // numero de estados para cada modelo HMM de subunidade
...
)
{
  ...
  // Alocando memoria
	bj = malloc(sizeof(double **)*HMM.n_par);
	for (i=0;i<HMM.n_par;i++)
		bj[i] = malloc(sizeof(double *)*(HMM.n_estados*length));
	for (i=0;i<HMM.n_par;i++)
		for (j=0;j<HMM.n_estados*length;j++)
			bj[i][j] = malloc(sizeof(double)*tamanho);

	Nm = malloc(sizeof(double ***)*HMM.n_par);
	for (i=0;i<HMM.n_par;i++)
		Nm[i] = malloc(sizeof(double **)*tamanho);
	for (i=0;i<HMM.n_par;i++)
		for (j=0;j<tamanho;j++)
			Nm[i][j] = malloc(sizeof(double *)*(HMM.n_estados*length));
	for (i=0;i<HMM.n_par;i++)
		for (j=0;j<tamanho;j++)
			for (k=0;k<HMM.n_estados*length;k++)
				Nm[i][j][k] = malloc(sizeof(double)*HMM.n_gaussianas[i]);
  ...
  for (i=0;i<HMM.n_par;i++)
    for (j=0;j<HMM.n_estados*length;j++)
      for (k=0;k<tamanho;k++)
        for (l=0;l<HMM.n_gaussianas[i];l++)
        {
          Nm[i][k][j][l] = b[i][j].c[l]*Gauss(b[i][j].m[l],HMM.ordem[i],b[i][j].v[l],x[i][k]);
          bj[i][j][k] += Nm[i][k][j][l];
        }
